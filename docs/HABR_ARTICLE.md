...
*[Skeleton Indexing](https://arxiv.org/abs/2502.09304) (KDD 2025) + [HippoRAG 2](https://arxiv.org/abs/2502.14802) (ICML 2025) + [VectorCypher](https://neo4j.com/docs/neo4j-graphrag-python/current/user_guide_rag.html) + Datalog Reasoning + 10 итераций оптимизации*
...
**Решение**: строим KNN-граф по embeddings чанков → [PageRank](https://en.wikipedia.org/wiki/PageRank) → извлекаем сущности только из top-25% "скелетных" чанков. Периферийные чанки привязываем через keyword matching.
...
**Решение**: два типа узлов в [Neo4j](https://neo4j.com/):
...
Гибридный retrieval в три фазы, вдохновлённый [VectorCypherRetriever](https://neo4j.com/docs/neo4j-graphrag-python/current/user_guide_rag.html) из Neo4j GraphRAG:

1. **Vector Index** → находим ближайшие PhraseNode через cosine similarity
2. **[Cypher](https://neo4j.com/docs/cypher-manual/current/introduction/) Traversal** → расширяем через RELATED_TO (до 3 хопов)
3. **PassageNode Collection** → собираем связанные пассажи → GraphContext

Ключевой инсайт: cosine re-ranking по реальным embeddings PassageNode из Neo4j бьёт [RRF-фьюжн](https://plg.uwaterloo.ca/~gvcormac/cormacksigir09-rrf.pdf).
...
|------|-------|------------|----------|
| 1 | **Mangle** ([Datalog](https://en.wikipedia.org/wiki/Datalog)) | 0.7 | 65 билингвальных ключевых слов |
...
Полная реимплементация [Google Mangle](https://github.com/google/mangle) (2,919 строк):

- [Lark](https://github.com/lark-parser/lark)-based парсер с кастомной грамматикой
- Semi-naive evaluation со [стратифицированным отрицанием](https://en.wikipedia.org/wiki/Stratification_(mathematics)#In_logic)
...
- **180 оценок** (30 x 6) через hybrid judge: embedding similarity + keyword overlap + [LLM-as-judge](https://arxiv.org/abs/2306.05685)
...
Попытка сделать judge "умнее" через [Chain-of-Thought](https://arxiv.org/abs/2201.11903) ("перечисли найденные ключевые слова → посчитай → выдай вердикт") вызвала регрессию с 144/180 до 48/180. GPT-4o-mini буквально искал английские строки в русском тексте. Простой промпт "match CONCEPTS, not strings" работает в 3 раза лучше.

### 4. Cosine re-ranking бьёт RRF

Hybrid search с [Reciprocal Rank Fusion](https://plg.uwaterloo.ca/~gvcormac/cormacksigir09-rrf.pdf) давал худшие результаты, чем cosine re-ranking по реальным embeddings из Neo4j. RRF хорош для combining разных сигналов, но когда оба сигнала — embedding-based, прямое cosine similarity точнее.
...
API: [FastAPI](https://fastapi.tiangolo.com/) REST (`/api/v1/`) + [FastMCP](https://github.com/jlowin/fastmcp) (SSE/MCP) — и для REST-клиентов, и для AI-агентов.
